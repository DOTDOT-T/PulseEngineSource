...
void InterfaceEditor::ShowFileGrid(const fs::path &currentDir, fs::path &selectedFile)
{
    ...
    for (const auto &entry : fs::directory_iterator(currentDir))
    {
        ...
        if (ImGui::BeginPopup("File Actions"))
        {
            ...
            {
                std::string fullPath = entry.path().string();
                if (fullPath.size() >= 5 && fullPath.substr(fullPath.size() - 5) == ".pmap")
                {
                ...
                if (fullPath.size() >= 8 && fullPath.substr(fullPath.size() - 7) == ".widget")
                {
                    if(ImGui::Selectable("Open widget"))
                    {
                        wdgtEditor->OpenNewWidget(fullPath.c_str());
                    }
                }
            }
            if (ImGui::Selectable("Delete"))
            {
                ...
                if (guid.empty() && !locatedAt)
                {
                    ...
                    EDITOR_INFO(
                        std::endl
                        << "Be aware that if the file was in fact in the engine it can cause issues."
                        << std::endl
                        << "If so, you have guid leak in the engine ressource.")
                }
                else
                {
                    EDITOR_SUCCESS("File " << fullPath << " have been deleted from engine ressources.")
                    EDITOR_INFO(std::endl
                                << "Be aware that object that refer to the guid[" << guid << "] will now have a nullptr received from engine ressource manager.")
